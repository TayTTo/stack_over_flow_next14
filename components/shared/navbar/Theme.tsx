...
import {themes} from "@/constants/constanst";

const Theme = () => {
    // extract mode and setMode by using useTheme hook
    const {mode, setMode} = useTheme();
    return (
        <Menubar className={"relative border-none bg-transparent shadow-none"}>
            <MenubarMenu>
                <MenubarTrigger
                    className={"focus:bg-light-900 data-[state=open]:bg-light-900 dark:focus:bg-dark-500 dark:data-[state=open]:bg-dark-200"}
                >
                    {mode === 'light' ? (
                        <Image
